...
export type ProceduralContext = { progress: number; checkpoint(progress: number): void; sceneId: string };

/** Adapts two independent interaction implementations to the shared lifecycle. */
export async function proceduralEngine(id: string): Promise<SceneEngine<ProceduralContext, SceneController>> {
  const module = await loadScene(id);
  return { create(host, context, options) {
    const canvas = document.createElement("canvas"), overlay = document.createElement("div");
    canvas.className = "scene-canvas"; overlay.className = "scene-overlay-host";
    ...
    const lost = (event: Event) => { event.preventDefault(); options.failed(Error("Graphics context lost")); };
    ...
    function setActive(value: boolean) {
      active = value; overlay.inert = !value;
      cancelAnimationFrame(frame); frame = 0;
      if (value && !disposed) { last = performance.now(); frame = requestAnimationFrame(tick); }
    }
    try { instance.start(); instance.update(0); setActive(active); options.ready(); }
    catch (error) { dispose(); throw error; }
    return { dispose, setActive, setReducedMotion(value) { reduced = value; } };
  } };
}
