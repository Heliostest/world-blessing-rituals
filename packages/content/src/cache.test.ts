...
describe("shared disposable cache", () => {
  it("promptly releases a cancelled queued reservation while another transfer is stalled", async () => {
    ...
    const b: Asset = { ...a, sha256: "b".repeat(64) },
      c: Asset = { ...a, sha256: "c".repeat(64) };
    ...
    const abort = new AbortController(),
      two = cache.fetch(b, "b", abort.signal);
    const caught = two.catch(() => "cancelled");
    await new Promise((resolve) => setTimeout(resolve, 5));
    abort.abort();
    const result = await Promise.race([
      caught,
      new Promise((resolve) => setTimeout(() => resolve("still queued"), 30)),
    ]);
  ...
  it("preserves offline hits and releases failed/cancelled reservations", async () => {
    ...
    controller.abort();
    await expect(
      s.cache.fetch(s.asset(2), "2", controller.signal),
    ).rejects.toThrow();
    expect((await s.cache.maintain({ clear: true })).bytes).toBe(0);
    const full = setup();
    full.fail();
    await expect(full.cache.fetch(full.asset(1), "1")).rejects.toThrow(
      "disk full",
    );
