...
export function createAssetCache<T>(
  ...
  let mutations: Promise<unknown> = Promise.resolve(),
    downloads: Promise<unknown> = Promise.resolve();
  let sequence = 0;
  ...
  async function prune(clear = false): Promise<CacheStats> {
    ...
    const reservedBytes = [...protectedSet.values()].reduce(
      (n, a) => n + (present.has(a.sha256) ? 0 : a.bytes),
      0,
    );
    let bytes = entries.reduce((n, e) => n + e.size, 0);
    const removed = new Set<string>();
    for (const e of entries.sort(
      (a, b) => a.used - b.used || a.key.localeCompare(b.key),
    )) {
    ...
    return {
      ...
      budget,
      reservedBytes,
      protectedBytes: entries
        .filter((e) => protectedSet.has(e.key))
        .reduce((n, e) => n + e.size, 0),
      overBudget: bytes + reservedBytes > budget,
      entries: entries.filter((e) => !removed.has(e.key)),
    };
  }
  const control: CacheControl = {
    protect(owner, raw) {
      return atomic(async () => {
        ...
        try {
          if (
            [...protectedAssets().values()].reduce((n, a) => n + a.bytes, 0) >
            budget
          )
            throw Error(
              "Scene cache budget exceeded; close another scene or increase budget",
            );
          await prune();
        } catch (error) {
          if (previous) owners.set(owner, previous);
    ...
    maintain(settings = {}) {
      return atomic(async () => {
        if (settings.budget !== undefined) {
          if (!Number.isSafeInteger(settings.budget) || settings.budget < 1)
            throw Error("Invalid cache budget");
  ...
  const read = (asset: Asset, signal?: AbortSignal) =>
    atomic(async () => {
      checkAbort(signal);
  ...
  return {
    ...
    async fetch(asset: Asset, url: string, signal?: AbortSignal): Promise<T> {
      ...
      const cancelled = new Promise<never>((_, reject) => {
        abort = () =>
          reject(new DOMException("Content load cancelled", "AbortError"));
