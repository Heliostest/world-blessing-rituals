...
function setup() {
  ...
  const manifest: SceneManifest = {
    ...
    revision: "v1",
    assets: {
      config: { path: "shared.json", bytes: 3, sha256: sha256(bytes) },
    },
    config: {},
  };
  const entry = {
  ...
  const io: ContentIO = {
    ...
    readHistory: async (key) => metadata.get(key),
    writeHistory: async (key, value) => {
    ...
    readManifest: async (url) => {
      ...
      return url.includes("catalog")
        ? {
            schemaVersion: 1,
            scenes: Array.from({ length: 500 }, (_, n) => ({
              ...entry,
              id: `s${n}`,
            })),
          }
    ...
    readAsset: async (a) => {
      transfers++;
      if (offline) throw Error("offline");
      files.set(a.sha256, bytes);
      return bytes;
    },
  };
  const make = () =>
    createSceneLibrary({ io, supports: (engine) => engine === "spring@1" });
  return {
    entry,
    manifest,
    io,
...
describe("on-demand scene library", () => {
  ...
  it("downloads on first open, hits cache on reopen, repairs evicted historical assets after restart", async () => {
    const s = setup();
    let lease = await s.library.open(s.entry);
    await lease.release();
    lease = await s.library.open(s.entry);
    await lease.release();
    expect(s.transfers()).toBe(1);
    s.files.clear();
    lease = await s.make().open(s.entry);
    await lease.release();
  ...
  it("opens saved complete content offline but reports an evicted offline scene as failure", async () => {
    const s = setup();
    const first = await s.library.open(s.entry);
    await first.release();
    s.offline();
    const offline = await s.make().open(s.entry);
  ...
  it("rejects unsupported engines before downloads and validates identity and bounded paths", async () => {
    const s = setup();
    await expect(
      s.library.open({ ...s.entry, engine: "future@1" }),
    ).rejects.toThrow(/upgrade/i);
    expect(s.manifests()).toBe(0);
    await expect(s.library.open({ ...s.entry, id: "other" })).rejects.toThrow(
      /identity/i,
    );
    expect(s.transfers()).toBe(0);
    expect(() =>
      parseCatalog(
        {
          schemaVersion: 1,
          scenes: [{ ...s.entry, manifestUrl: "javascript:alert(1)" }],
        },
        "https://cdn.test/catalog.json",
      ),
    ).toThrow();
  });
  it("does not publish ready on cancellation and releases the whole dependency lease", async () => {
    ...
    await expect(
      s.library.open(s.entry, { signal: cancel.signal }),
    ).rejects.toThrow();
    expect(s.protectedOwners.size).toBe(0);
  });
});
