...
export type SceneProgress = {
  phase: "manifest" | "downloading" | "ready";
...
const token = (v: unknown) =>
  typeof v === "string" && /^[a-zA-Z0-9][a-zA-Z0-9._@-]{0,95}$/.test(v);
...
export function parseCatalog(raw: unknown, base: string): CatalogEntry[] {
  if (
    ...
    raw.scenes.length > 5000
  )
    throw Error("Invalid scene catalog");
  ...
  return raw.scenes.map((e: unknown) => {
    ...
    return {
      ...
      engine: e.engine,
      revision: e.revision,
      manifestUrl: contentURL(e.manifestUrl, base),
    };
  });
}
export function parseSceneManifest(raw: unknown): SceneManifest {
  if (
    ...
    !object(raw.assets) ||
    !object(raw.config) ||
    Object.keys(raw.assets).length > 128 ||
    JSON.stringify(raw.config).length > 65536
  )
  ...
  for (const [name, value] of Object.entries(raw.assets)) {
    ...
    if (hashes.has(asset.sha256) && hashes.get(asset.sha256) !== asset.bytes)
      throw Error("Conflicting asset sizes");
    hashes.set(asset.sha256, asset.bytes);
    assets[name] = asset;
  }
  return {
    schemaVersion: 1,
...
const entryKey = (entry: CatalogEntry) =>
  `scene-manifest:${sha256(new TextEncoder().encode(JSON.stringify([entry.id, entry.engine, entry.revision, entry.manifestUrl])).buffer)}`;
let nextOwner = 0;
export function createSceneLibrary(options: {
  ...
  async function saved(entry: CatalogEntry) {
    try {
      const manifest = parseSceneManifest(
        await io.readHistory(entryKey(entry)),
      );
  ...
  return {
    ...
    async status(
      entry: CatalogEntry,
      hashes: Set<string>,
    ): Promise<"unknown" | "cached" | "missing"> {
      const manifest = await saved(entry);
      if (!manifest) return "unknown";
      return Object.values(manifest.assets).every((a) => hashes.has(a.sha256))
        ? "cached"
        : "missing";
    },
    async open(
      ...
      settings: {
        signal?: AbortSignal;
        onProgress?(value: SceneProgress): void;
      } = {},
    ): Promise<SceneLease> {
      const { signal } = settings;
      checkAbort(signal);
      if (!options.supports(entry.engine))
        throw Error("Please upgrade the app for this scene engine");
      ...
      const manifest =
        (await saved(entry)) ??
        parseSceneManifest(await io.readManifest(url, signal));
      checkAbort(signal);
      match(entry, manifest);
      const assets = [
        ...new Map(
          Object.values(manifest.assets).map((a) => [a.sha256, a]),
        ).values(),
      ];
      ...
      try {
        ...
        return {
          ...
          async read(name, readSignal) {
            ...
            if (!bytes)
              throw Error("Scene resource was removed; retry to download");
            verifyAsset(asset, bytes);
            return bytes;
          },
        };
