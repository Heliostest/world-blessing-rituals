...
export function createWebContentIO(): ContentIO {
  // One DOM host may have multiple clients (catalog, legacy woodfish, HMR).
  // They must share live reservations as well as the same IndexedDB files.
  const shared = globalThis as typeof globalThis & {
    __wbrSceneContentIO?: ContentIO;
  };
  if (shared.__wbrSceneContentIO) return shared.__wbrSceneContentIO;
  ...
  async function transaction<T>(
    ...
    return new Promise((resolve, reject) => {
      ...
      tx.onerror = tx.onabort = () => {
        clearTimeout(timer);
        reject(tx.error ?? Error("Cache transaction failed"));
      };
      try {
        run(tx, (v) => {
          value = v;
  ...
  const cache = createAssetCache<ArrayBuffer>({
    ...
    touch: (key, used) =>
      transaction<void>(["asset-index"], "readwrite", (tx) => {
        const store = tx.objectStore("asset-index"),
          request = store.get(key);
        request.onsuccess = () => {
          if (request.result) store.put({ ...request.result, used });
        };
      }),
    remove,
  ...
  return (shared.__wbrSceneContentIO = {
    cache,
    readCachedAsset: async (asset, signal) => {
      try {
        return await cache.read(asset, signal);
      } catch {
        checkAbort(signal);
        return;
      }
    ...
    readManifest: async (url, signal) =>
      JSON.parse(
        new TextDecoder().decode(
          await readBytes(url, 2 * 1024 * 1024, signal, 5000),
        ),
      ),
