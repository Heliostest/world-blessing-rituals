...
const bytes = await readFile(new URL("../assets/woodfish/blender-v2/woodfish.glb", import.meta.url));
assert.equal(bytes.toString("ascii", 0, 4), "glTF");
assert.equal(bytes.readUInt32LE(4), 2);
assert.equal(bytes.readUInt32LE(8), bytes.length);
assert.ok(bytes.length < 12 * 1024 * 1024, "Mobile GLB file budget");
...
assert.deepEqual(gltf.nodes.map((n) => n.name).sort(), ["Mallet", "WoodfishBody"]);
for (const node of gltf.nodes) {
  assert.deepEqual(node.translation ?? [0, 0, 0], [0, 0, 0], "Export at animation origin, not studio preview placement");
  assert.deepEqual(node.rotation ?? [0, 0, 0, 1], [0, 0, 0, 1]);
  assert.deepEqual(node.scale ?? [1, 1, 1], [1, 1, 1]);
  assert.ok(!node.matrix, "Bake model transforms into geometry for strike raycasts");
}

const widths = { SCALAR: 1, VEC2: 2, VEC3: 3, VEC4: 4 };
const components = { 5126: [4, "readFloatLE"], 5125: [4, "readUInt32LE"], 5123: [2, "readUInt16LE"] };
function accessor(index) {
  const a = gltf.accessors[index], view = gltf.bufferViews[a.bufferView];
  const [size, read] = components[a.componentType];
  const width = widths[a.type], stride = view.byteStride ?? size * width;
  const offset = (view.byteOffset ?? 0) + (a.byteOffset ?? 0);
  return Array.from({ length: a.count }, (_, i) =>
    Array.from({ length: width }, (_, j) => bin[read](offset + i * stride + j * size)));
}
let triangles = 0;
for (const mesh of gltf.meshes) {
  ...
  const normals = accessor(a.NORMAL), tangents = accessor(a.TANGENT), uvs = accessor(a.TEXCOORD_0);
  for (let i = 0; i < positions.length; i++) {
    assert.ok(positions[i].every(Number.isFinite));
    assert.ok(Math.abs(Math.hypot(...normals[i]) - 1) < .002);
    assert.ok(Math.abs(Math.hypot(...tangents[i].slice(0, 3)) - 1) < .002);
    assert.ok(Math.abs(tangents[i][3]) === 1);
    assert.ok(uvs[i].every((v) => v >= -1e-5 && v <= 1.00001), "UVs stay inside atlas");
  }
  ...
  for (let i = 0; i < indices.length; i += 3) {
    ...
    for (let j = 0; j < 3; j++) {
      ...
      entry.count++; entry.winding += x < y ? 1 : -1; edges.set(key, entry);
    }
  }
...
assert.ok(triangles < 100000, "Combined mobile geometry budget");
for (const material of gltf.materials) {
  ...
  assert.ok(!material.doubleSided, "A closed carved shell needs no backface shortcut");
}
for (const image of gltf.images) {
  assert.ok(!image.uri && Number.isInteger(image.bufferView), "Textures embedded for offline use");
}
console.log(`Blender GLB verified: ${triangles} triangles, UVs/tangents, closed shells, embedded PBR + AO, ${(bytes.length / 1048576).toFixed(2)} MiB.`);
